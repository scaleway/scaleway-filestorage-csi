...
const (
	instanceCommercialType = "POP2-8C-32G"
	instanceImageLabel     = "ubuntu_noble"
	remoteCSIPath          = "/usr/local/bin/scaleway-filestorage-csi"
...
var _ = Describe("Sanity", func() {
	It("should run sanity test successfully", func(ctx SpecContext) {
		...
		Expect(goBuild(outDriver, "../../cmd/scaleway-filestorage-csi/main.go")).Should(Succeed())
		DeferCleanup(func() {
		...
		By("Running scaleway-filestorage-csi on instance")
		driverEnv := map[string]string{
			scw.ScwDefaultRegionEnv:    os.Getenv(scw.ScwDefaultRegionEnv),
		...
		sanityResult, err := client.RunCmd(ctx, fmt.Sprintf(
			"%s --csi.endpoint=unix:/tmp/csi.sock --csi.testvolumesize=%d --csi.testvolumeexpandsize=%d"+
				" --csi.testnodevolumeattachlimit=false --ginkgo.no-color"+
				" --ginkgo.skip=\"should not fail when creating volume with maximum-length name\"",
			remoteSanityPath, driver.MinVolumeSize, 2*driver.MinVolumeSize,
		), nil)
		Expect(err).NotTo(HaveOccurred())
