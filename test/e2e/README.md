...
- A Kubernetes (v1.20+) cluster running with the `scaleway-filestorage-csi` already installed.
...
4. Run the tests:
    ...
    ```console
    ./e2e.test -ginkgo.focus='External.Storage.*filestorage.csi.scaleway.com.*' -ginkgo.skip='Ephemeral-volume' -ginkgo.timeout=6h -storage.testdriver=test-driver.yaml
    ```
